...
Come già accennato gran parte del vocabolario principale di FoodOn deriva dalla trasformazione di LanguaL, un dizionario di indicizzazione degli alimenti maturo e popolare.
...
\index{Similitudini}
Come LanguaL, FoodOn fa la stessa distinzione tra le piante di origine alimentare e gli organismi animali che si trovano in LanguaL Facet B e gli schemi di categorizzazione degli alimenti degli enti governativi in LanguaL Facet B i quali sono tutti copiati termine per termine in FoodOn.
Sotto il ramo ``food product type" c'è una nuova gerarchia ``foodon food product" che è una copia molto approssimativa del ramo prodotti alimentari del FDA Code of Federal Regulations (CFR), ma riorganizzata per essere la più ontologicamente ``pura".
La maggior parte degli articoli di ``foodon food product" sono collegati direttamente o per eredità tramite un riferimento ``member of" ad una categoria CFR.
L'intera LanguaL Facet B è duplicata sotto il ramo ``food source" di FoodOn ed un riferimento a un organismo in Facet B non richiede che tale organismo svolga un ruolo alimentare.
...
In primo luogo in LanguaL esistono casi in cui sono presenti termini del tipo ``[facet X] not used" o ``[facet X term] not known" come, ad esempio, ``Mezzo di imballaggio non utilizzato".
...
Ciò permette a un ragionatore di sapere che l'entità descritta è un \textit{taglio di carne}, il che consente che una sottoclasse di \textit{ taglio di carne} sia pertinente.

LanguaL per indicare l'ingrediente principale di qualsiasi alimento indicizzato utilizza ``food source" e per indicare gli ingredienti secondari ``treatment applied > ingredient added".
Grazie alle ontologie OWL che permettono di usare le proprietà degli oggetti per descrivere gli ingredienti primari e secondari, FoodOn può utilizzare relazioni del tipo ``has ingredient" e ``has substance added" per fare ciò.

Per regolamentare LanguaL, FoodOn ha conservato una serie di contenuti aggiunti al cibo.
Tuttavia le relazioni ``has substance added" di un prodotto alimentare potrebbero contenere delle percentuali o delle proporzioni, il che avvicini gli alimenti composti a una ricetta o a una descrizione dell'etichetta del prodotto.
