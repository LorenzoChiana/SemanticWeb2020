...
FoodOn da la possibilità di collegare due prodotti o ingredienti di origine alimentare attraverso la relazione ``\textit{has food substance analog}" permettendo così di indicare i possibili sostituti, per l'uso in analisi e in applicazioni sensibili agli allergeni e/o ad altri vincoli dietetici, di un particolare prodotto o ingrediente.
...
Uno degli obiettivi di FoodOn è quello di diventare uno standard ben documentato, curato e stabile e
...
\caption{Visualizzazione di un albero FoodOn di 15 rami}
\label{fig:reuse2}
\end{figure}

Il riutilizzo dei termini fra le varie ontologie consente di condividere lo sforzo di fornire un vocabolario standardizzato.
...
Quindi la definizione può essere risolta affermando direttamente:
\begin{center}
...
Tuttavia viene incoraggiato il riutilizzo dell'ontologia anche per chi non possiede una formazione completa nell'ambito del Web Semantico fornendo fogli di calcoli o modelli di database personalizzabili tramite portali web.
Inoltre, Hsiao Lab sta sviluppando uno strumento che trasforma i contenuti ontologici di markup in standard, forniti sia in una form web che in versione tabulare o json per l'implementazione nei sistemi di gestione e scambio dati.
\newpage
...
FoodOn utilizza le seguenti relazioni:
\begin{itemize}
...
\end{itemize}
\item ``\textbf{has substance added}", proprietà di un oggetto che può essere applicata a una qualsiasi sostanza (chimica, fisica, alimentare, ...). Si differenzia da ``has part" perché la sostanza aggiunta potrebbe non essere individuabile nella miscela in un secondo momento.
